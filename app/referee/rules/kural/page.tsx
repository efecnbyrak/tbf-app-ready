...
import { FileText, Download, ExternalLink, Loader2, Book, Hash, ChevronRight } from "lucide-react";
...
export default function KuralPage() {
    ...
    const digitalContent = selectedRule?.url ? null : parseDigitalContent(selectedRule?.content || null);
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {rules.map((rule) => (
                    <button
                        ...
                        className={`group p-5 rounded-2xl border-2 text-left transition-all relative overflow-hidden ${selectedRule?.id === rule.id
                            ? "border-red-600 bg-red-50/50 dark:bg-red-900/10 shadow-lg shadow-red-100 dark:shadow-none"
                            : "border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700 bg-white dark:bg-zinc-900"
                            }`}
                    >
                        <div className="flex items-start gap-4">
                            <div className={`p-3 rounded-xl ${selectedRule?.id === rule.id ? "bg-red-600 text-white" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-400 group-hover:text-zinc-600"} transition-colors`}>
