...
export function OfficialAvailabilityForm({ referee, days, existingForm, isLocked, deadline, startDate, endDate, customRoleLabel, customRoleTitle }: OfficialAvailabilityFormProps) {
    ...
    useEffect(() => {
        if (state?.success) {
            window.alert('Uygunluk formunuz başarıyla kaydedildi!');
        }
    }, [state?.success]);
        ...
        <form action={handleSubmit} className="space-y-8 relative overflow-hidden rounded-3xl">
            {isLocked && (
                <div className="relative z-[40] mb-8 animate-in slide-in-from-top-4 duration-500">
                    <div className="bg-white/90 dark:bg-zinc-900/90 backdrop-blur-xl px-8 py-10 rounded-[2.5rem] shadow-2xl border-4 border-red-600/50 flex flex-col items-center gap-4 group hover:scale-[1.02] transition-transform">
                        <div className="w-20 h-20 bg-red-600 rounded-2xl flex items-center justify-center shadow-lg shadow-red-600/40 rotate-3 group-hover:rotate-0 transition-transform">
                            <Lock className="w-10 h-10 text-white" />
                        </div>
                        <div className="text-center">
                            <h3 className="text-3xl font-black text-zinc-900 dark:text-white mb-1 uppercase tracking-tighter italic">ERİŞİM KAPALI</h3>
                            <p className="text-zinc-600 dark:text-zinc-400 font-bold text-sm tracking-tight px-4 uppercase opacity-80">
            ...
            {state?.success && (
                <div className="bg-emerald-100 border border-emerald-200 text-emerald-700 px-4 py-3 rounded-2xl relative font-bold uppercase italic text-sm animate-in fade-in slide-in-from-top-2 duration-300">
                    Başarıyla kaydedildi.
                </div>
            )}

            <div
                style={isLocked ? { filter: 'blur(8px) grayscale(1)', pointerEvents: 'none', userSelect: 'none' } : {}}
