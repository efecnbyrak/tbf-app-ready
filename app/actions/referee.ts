...
export async function updateRefereeProfile(formData: FormData) {
    ...
    // Validate
    const validatedFields = ProfileSchema.safeParse({ email, phone, address, iban, securityQuestion, securityAnswer });
    if (!validatedFields.success) {
        const fieldErrors = validatedFields.error.flatten().fieldErrors;
        return { error: fieldErrors.email?.[0] || fieldErrors.phone?.[0] || fieldErrors.securityQuestion?.[0] || fieldErrors.securityAnswer?.[0] || "Geçersiz veri." };
    }
    ...
    try {
        ...
        // Update security questions in User model
        await db.user.update({
            where: { id: session.userId },
            data: {
