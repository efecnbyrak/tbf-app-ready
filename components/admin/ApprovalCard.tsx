...
export function ApprovalCard({ user }: ApprovalCardProps) {
    ...
    const handleApprove = async () => {
        if (!confirm(`${user.referee?.firstName} ${user.referee?.lastName} isimli kullanıcıyı onaylamak istediğinize emin misiniz?`)) return;
        setLoading('approve');
        try {
    ...
    const handleReject = async () => {
        if (!confirm(`${user.referee?.firstName} ${user.referee?.lastName} isimli kullanıcıyı REDDETMEK istediğinize emin misiniz? Bu işlem geri alınamaz ve kullanıcı silinecektir.`)) return;
        setLoading('reject');
        try {
    ...
    return (
        <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border-2 border-zinc-100 dark:border-zinc-800 p-6 space-y-5 hover:border-red-100 dark:hover:border-red-900/30 transition-all">
            ...
            <div className="space-y-2.5 text-sm p-4 bg-zinc-50 dark:bg-zinc-950 rounded-xl border border-zinc-100 dark:border-zinc-800">
                ...
                <div className="flex justify-between items-center">
                    ...
                    <span className="font-bold text-zinc-900 dark:text-zinc-100">{user.referee?.phone}</span>
                </div>
                <div className="flex justify-between items-center">
                    <span className="text-zinc-500 font-medium text-xs uppercase">E-posta:</span>
                    <span className="truncate max-w-[150px] font-bold text-zinc-900 dark:text-zinc-100">{user.referee?.email}</span>
                </div>
                <div className="flex justify-between items-center">
