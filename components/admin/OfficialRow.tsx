...
import { UserSquare2, ChevronRight, ShieldCheck, Activity, UserX, UserCheck, MapPin, ShieldX } from "lucide-react";
...
export const OfficialRow = memo(function OfficialRow({ official, onClick, onToggleActive, onPromote, onDemote, isSuperAdmin }: OfficialRowProps) {
    ...
    const cityName = formatLocation(official.regions, official.address);
        ...
        >
            ...
            <div className="flex items-center gap-4 flex-1 min-w-0">
                <div className="relative w-14 h-14 rounded-2xl bg-zinc-100 dark:bg-zinc-800 shrink-0 border-2 border-zinc-200 dark:border-zinc-700 overflow-hidden shadow-inner">
                    {official.imageUrl ? (
                        <img src={official.imageUrl} alt={official.firstName} className="w-full h-full object-cover" loading="lazy" />
                    ) : (
                        <img src="/hakem/defaultHakem.png" alt="Default" className="w-full h-full object-cover" loading="lazy" />
                    )}
                    {isSuspended && (
            ...
            <div className="flex items-center justify-between sm:justify-end gap-6 pt-3 sm:pt-0 border-t sm:border-t-0 border-zinc-100 dark:border-zinc-800">
                <div className="flex flex-col sm:items-end">
                    <div className="flex items-center gap-1.5 text-zinc-400">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-wider">{cityName}</span>
                    </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    {onDemote && official.user?.role?.name === "ADMIN" && (
                        <button
                            onDoubleClick={(e) => { e.stopPropagation(); onDemote(); }}
                            title="Yöneticiliği Al (Çift Tıklayın)"
                            className="p-3 rounded-2xl transition-all text-indigo-400 hover:text-indigo-600 bg-indigo-50 dark:bg-indigo-900/20"
                        >
